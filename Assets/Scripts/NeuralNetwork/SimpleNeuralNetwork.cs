...
public class SimpleNeuralNetwork
{
    ...
    private readonly static double DEFAULT_INPUTS_WEIGHT = 1;
    ...
    {
        List<INeuron> neurons = _layers.First().Neurons;
    ...
    public void UpdateWeightsLayers(double[] weights)
    {
        for (int k = _layers.Count - 1; k > 0; k--)
        {
            _layers[k].Neurons.ForEach(neuron =>
            {
                neuron.Inputs.ForEach(connection =>
                {
                    connection.UpdateWeight(weights[k]);
                });
            });
        }
    }
